...
class FormProcedureInputUserController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->filter(function($filter){
            ...
            $filter->in('user.region_id', 'Wilayah')->checkbox(Region::get()->pluck('name', 'id')->toArray());
            $filter->in('form_procedure.procedure_id', 'SOP')->checkbox(Procedure::get()->pluck('title', 'id')->toArray());
        });
        $grid->column('user.name', __('User Akun'));
        $grid->column('user.region_name', __('Wilayah'));
